...
enum ModelError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .unknwonedError: return "예상하지 못한 에러 발생"
        }
    }
    var recoverSuggestion: String? {
        switch self {
        case .fetchSettingError, .fetchRecordError, .createInitSettingError:
            return "페이지를 새로고침해주세요. 문제가 지속될 경우 앱을 다시 시작해주세요."
        case .updateDataError:
            return "다시 시도해주세요. 문제가 지속될 경우 앱을 다시 시작해주세요."
        case .unknwonedError:
            return "앱을 다시 시작 해주세요."
        }
    }
}
